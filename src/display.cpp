...
void display_show_msg(uint8_t *image_buffer, MSG message_type, const char *message_text)
{
    ...
    case FW_UPDATE:
    {
        const char string1[] = "TRMNL enhanced by jakubjirak.com: firmware update, starting now...";
        bbep.getStringBox(string1, &rect);
#ifdef __BB_EPAPER__
        bbep.setCursor((bbep.width() - rect.w) / 2, 400);
#else
    ...
    {
        const char string1[] = "jakubjirak.com firmware update failed. Device will restart...";
        bbep.getStringBox(string1, &rect);
#ifdef __BB_EPAPER__
        bbep.setCursor((bbep.width() - rect.w) / 2, 400);
#else
    ...
    {
        const char string1[] = "jakubjirak.com firmware updated. Device will restart...";
        bbep.getStringBox(string1, &rect);
#ifdef __BB_EPAPER__
        bbep.setCursor((bbep.width() - rect.w) / 2, 400);
#else
